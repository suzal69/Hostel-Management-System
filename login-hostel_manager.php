<?php 
// Ensure the session is started at the very top
...
<html lang="en">
<head>
    <title>HMS | Manager Login</title>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta name="keywords" content="Hostel Manager Login" />
...
<style>
    ...
    <style>
        /* ADDED: Global Box-Sizing for consistency */
        *, *::before, *::after {
        ...
        body {
            ...
            /* background-image: url('web_profile/images/index.jpg'); */
        ...
        /* Specific styles for the icon padding */
        .form-input.with-icon {
            padding-left: 2.5rem; /* 40px for the icon */
            /* ADDED: Ensure input itself respects border-box model */
            box-sizing: border-box; 
        }
        /* ADDED: Ensure the login card container has a specific width if it's based on the Tailwind max-w-md class */
        .login-container {
             /* Set a fixed maximum width for the card to ensure inputs don't stretch beyond it */
            max-width: 448px; /* max-w-md equivalent in Tailwind */
            width: 100%;
        }
    </style>
</head>
...
<body>
    ...
    <!-- server alert placeholder (moved inside login container for correct z-index) -->
    
    <div class="login-container">
        
        <div class="header-section">
            <h1>Hostel Management System</h1>
            <p>Hostel-Manager/Admin Login</p>
        </div>

        <?php
        // Server-side alert banner (uses web/css/login_style.css)
        $serverMsg = '';
        $serverType = 'error';
        if (isset($_GET['error'])) {
            switch ($_GET['error']) {
                case 'emptyfields': $serverMsg = 'Please fill in all fields.'; break;
                case 'invalidusername': $serverMsg = 'Invalid username format.'; break;
                case 'wrongpwd': $serverMsg = 'Incorrect password. Please try again.'; break;
                case 'nouser': $serverMsg = 'No user found with that username.'; break;
                case 'sqlerror': $serverMsg = 'Unexpected database error. Try again later.'; break;
                case 'invalidaccess': $serverMsg = 'Access denied. Please use the login form.'; break;
        ...
        <form id="hmLoginForm" action="includes/login-hm.inc.php" method="POST" class="login-form">
            ...
            <div class="form-group">
                ...
                <div class="mt-1 relative rounded-md shadow-sm">
                    ...
                    <input type="text" id="username" class="form-input with-icon" name="username" placeholder="Username" required 
                        value="<?php echo isset($_GET['error']) ? htmlspecialchars($_POST['username'] ?? '') : ''; ?>" />
                </div>
            ...
            <div class="form-group">
                ...
                <div class="mt-1 relative rounded-md shadow-sm">
                    ...
                    <input type="password" id="pwd" class="form-input with-icon" name="pwd" placeholder="Password" required />
                </div>
        ...
        <div class="links-section">
            <p>
                Not a member yet?
                <a href="signup_hm.php">Sign Up</a>
            </p>
            <p>
                Login as 
    ...
    <script>
        // Client-side validation removed to rely on server-side checks.
    </script>
</body>
</html>